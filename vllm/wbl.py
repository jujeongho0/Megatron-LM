...
@CustomOp.register("wbl_rotary_embedding")
class WBLRotaryEmbedding(RotaryEmbedding):
    ...
    def _compute_inv_freq(self, base: float) -> torch.Tensor:
        ...
        inv_freq /= self.scale_inv_freq
        return inv_freq


@CustomOp.register("wbl_deepseek_scaling_rotary_embedding")
class WBLDeepseekScalingRotaryEmbedding(DeepseekScalingRotaryEmbedding):
...
def get_rope(
    ...
    if not rope_scaling:
        rotary_emb = WBLRotaryEmbedding(
            head_size, rotary_dim, max_position, base, scale_inv_freq, is_neox_style, dtype
        )
    else:
        ...
        if scaling_type == "deepseek_yarn":
            ...
            rotary_emb = WBLDeepseekScalingRotaryEmbedding(
                head_size,
                rotary_dim,
...
class WBLAttention(nn.Module):
    def __init__(
        ...
        if self.q_lora_rank is not None:
            ...
            self.q_a_layernorm = GemmaRMSNorm(self.q_lora_rank, eps=config.rms_norm_eps)
        ...
        self.kv_a_layernorm = GemmaRMSNorm(self.kv_lora_rank, eps=config.rms_norm_eps)
...
class WBLDecoderLayer(nn.Module):
    def __init__(
        ...
        self.pre_mlp_layernorm = GemmaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_mlp_layernorm = GemmaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class WBLMixtureOfExperts(MixtureOfExperts):
    ...
    def extract_moe_parameters(self, example_moe: WBLMoE | None):
        if example_moe is None:
            ...
            logger.warning("WBL: No WBLMoE layer found in model.layers.")
        else:
...
class WBLForCausalLM(
    nn.Module, SupportsPP, WBLMixtureOfExperts, SupportsLoRA, SupportsEagle
):
...
def get_spec_layer_idx_from_weight_name(
    config: WBLConfig, weight_name: str
) -> int | None:
    if (
